...
void TauEtStudy() {
    ...
    t->SetBranchAddress("TauCluster_EM", &tauEM);
    t->SetBranchAddress("TauCluster_Had", &tauHad);
	...
	// cout << "Initialized L2Et[4][1]:" << endl;
	...
	TH1F* h2 = new TH1F("reco2 - true", "Reco 2 Et - True Et; Et (GeV); Entries", 100, -100., 50.);
	TH1F* h3 = new TH1F("recoall - true", "Reco All Et - True Et; Et (GeV); Entries", 100, -100., 50.);

	// Create histogram for true - reconstructed vs true energies
	TH2F* h4 = new TH2F("reco1 - true vs reco1", "Reco 1 Et - True Et vs. True Et; True Et (GeV); Reco 1 Et - True Et (GeV)", 100, 0., 120., 100, -100., 75.);
	...
	for (Int_t i = 0; i < nentries; i++) {
		if (TrueEt[i] > 20.) {
			...
			h4->Fill(TrueEt[i], Reco_1_Et[i] - TrueEt[i]);
		}
	}
